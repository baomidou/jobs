 ...
 */
@Slf4j
public class XxlJobScheduleHelper {
    private static XxlJobScheduleHelper instance = new XxlJobScheduleHelper();

    public static XxlJobScheduleHelper getInstance() {
        return instance;
    }
    ...
    public void start() {
        ...
        scheduleThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while (!toStop) {
                    ...
                    try {
                        if (conn == null || conn.isClosed()) {
                            conn = XxlJobHelper.getDataSource().getConnection();
                        }
                        ...
                        List<XxlJobInfo> scheduleList = XxlJobHelper.getJobInfoService().scheduleJobQuery(maxNextTime);
                        if (scheduleList != null && scheduleList.size() > 0) {
                            ...
                            for (XxlJobInfo jobInfo : scheduleList) {
                                ...
                                log.debug(">>>>>>>>>>> xxl-job, push time-ring : " + ringSecond + " = " + Arrays.asList(ringItemData));
                            }

                            // 3、更新trigger信息
                            for (XxlJobInfo jobInfo : scheduleList) {
                                XxlJobHelper.getJobInfoService().updateById(jobInfo);
                            }
                        ...
                        if (!toStop) {
                            log.error(">>>>>>>>>>> xxl-job, XxlJobScheduleHelper#scheduleThread error:{}", e);
                        }
                    } finally {
                        if (conn != null) {
                            try {
                ...
                log.info(">>>>>>>>>>> xxl-job, XxlJobScheduleHelper#scheduleThread stop");
            }
        });
        scheduleThread.setDaemon(true);
        scheduleThread.setName("xxl-job, admin XxlJobScheduleHelper#scheduleThread");
        ...
        ringThread = new Thread(new Runnable() {
            ...
            public void run() {
                    ...
                    try {
                        ...
                        if (ringItemData != null && ringItemData.size() > 0) {
                            ...
                            for (int jobId : ringItemData) {
                                // do trigger
                                XxlJobTriggerPool.trigger(jobId, TriggerTypeEnum.CRON, -1, null, null);
                            }
                            // clear
                            ringItemData.clear();
                        }
                        ...
                        if (!toStop) {
                            log.error(">>>>>>>>>>> xxl-job, XxlJobScheduleHelper#ringThread error:{}", e);
                        }
                    }
                ...
                log.info(">>>>>>>>>>> xxl-job, XxlJobScheduleHelper#ringThread stop");
            }
        });
        ringThread.setDaemon(true);
        ringThread.setName("xxl-job, admin XxlJobScheduleHelper#ringThread");
        ringThread.start();
    }
